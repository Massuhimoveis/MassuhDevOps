...
function compareAndSetStatusIcon(element, clienteValue, imovelValue) {
  const parametro = element.data("parametro");
  console.log("Comparing values for:", parametro);
  ...
  if (normalizedClienteValue === "" || normalizedImovelValue === "") {
    ...
    if (
      ...
      console.log(
        `Comparing ${parametro}: Cliente Value: ${numClienteValue}, Imovel Value: ${numImovelValue}`
      );

      if (!isNaN(numClienteValue) && !isNaN(numImovelValue)) {
        // Specific condition for "Quartos-status"
        if (
          parametro === "Quartos-status" ||
          parametro === "Banheiros-status" ||
          parametro === "Vagas de Garagem-status"
        ) {
          console.log(
            `Checking Quartos-status: numImovelValue (${numImovelValue}) >= numClienteValue (${numClienteValue})`
          );

          isMatch = numImovelValue >= numClienteValue;
          console.log("Quartos-status Match:", isMatch);
        } else {
          isMatch = numClienteValue === numImovelValue;
          console.log(`Numeric comparison result: ${isMatch}`);
        }
      } else {
        ...
        console.log(`String comparison result: ${isMatch}`);
      }
    }
  }
...
function compararClienteImovel2(element, clienteMomento) {
  ...
  try {
    ...
    // Inserir informações do imóvel clicado (element is now the actual object)
    const imovelClicado = element;
    console.log("Imóvel Clicado:", imovelClicado);

    // Extract relevant fields from the object
    const imgSrc = imovelClicado.picture_link || "Imagem não disponível";
    const metragem = imovelClicado.metragem || "N/A";
    const bairro = imovelClicado.bairro || "Bairro não informado";
    const valor = imovelClicado.valor || "Valor não disponível";
    const tabela = $("#tabelaComparacao2");
    // Ensure the table is visible
    // Ensure table and parent containers are visible
    $(".recomendacaoViewImovel2").css("display", "flex").show(); // Ensure the container is visible
    console.log("Table visibility after forcing show:", tabela.is(":visible"));
    ...
    // Update the recommendation section with the extracted values
    $("#metragemImovel-recomendacao").text(metragem);
    $("#bairroImovel-recomendacao").text(bairro);
    $("#valorImovel-recomendacao").text(valor);

    // Set the URL of the selected property for future use
    let imovelURLComparativo = imovelClicado.url_imovel;
    imovelSelected = imovelURLComparativo;
    console.log("URL do Imóvel:", imovelURLComparativo);

    // Fetch comparison data for the selected property
    buscarImoveis((imovelData) => {
      ...
      if (imovelData.data && imovelData.data.length > 0) {
        ...
        // Create a new list item for comparison
        ...
        // Iterate through each row for comparison
        rows.forEach((row, index) => {
          ...
          // Fetch values for comparison
          const clienteValue = clienteMomento[clientParam] || "";
          const imovelValue = imovel[imoveisParam] || "";

          // Remove the row if the client value is empty (excluding the first row)
          if (clienteValue === "" && rowElement.index() !== 0) {
            rowElement.remove();
            return; // Skip further processing for this row
          }

          // Update client data in the comparison table
          if (clienteMomento.hasOwnProperty(clientParam)) {
            cellsClient.text(clienteMomento[clientParam]);
          }

          // Update property data in the comparison table and handle url_imovel
          if (
            imoveisParam === "url_imovel" &&
            imovel.hasOwnProperty(imoveisParam)
          ) {
            // If it's the URL field, add a hyperlink
            const url = imovel[imoveisParam];
            cellsImoveis.html(`<a href="${url}" target="_blank">${url}</a>`);
          } else if (imovel.hasOwnProperty(imoveisParam)) {
            // For other fields, set the text content
            cellsImoveis.text(imovel[imoveisParam]);
          }

          // Compare values and set the status icon
          console.log(
        ...
        // Append the comparison list item to the table
        console.log("Appending listItem to tabelaComparacao...");
        tabela.append(listItem);
        console.log("tabela:", tabela.html());
        console.log("Tabela atualizada com sucesso.");
      }
    }, `url_imovel=${imovelURLComparativo}`);
  } catch (error) {
    console.error("Error occurred in compararClienteImovel2 function:", error);
  }
}
